...
# 添加列名
header_row = ['diagnosis_result', 'radius', 'texture', 'perimeter', 'area', 'smoothness', 'compactness', 'symmetry','fractal_dimension']

# 载入数据
heart = pd.read_csv('KNN_cancer1001.csv', names=header_row)
...
# 癌症（0=良性，1=恶性）
heart["diagnosis_result"] = np.where(heart["diagnosis_result"] != 0, 1 , 0)
label = heart.diagnosis_result
print(label)
# In[47]:
...
import joblib
# 保存模型
joblib.dump(clf, '癌症KNN.m')

# In[53]:


# 提取算法
knn_digits = joblib.load("癌症KNN.m")
#
# In[54]:
